...
Rwanda's life expectancy went from 48.3 years in 2000 to 66.1 years in 2015. Almost doubling in fifteen years. That doesn't happen by accident, it comes from decisions on HIV treatment, immunization, schooling, and income growth. This project builds a regression model that learns which of those factors actually move life expectancy using WHO data from 193 countries, so the same reasoning behind Rwanda's turnaround can be applied to any developing country trying to figure out where to invest next.

## Dataset

WHO Life Expectancy dataset: 2938 rows, 193 countries, 2000 to 2015, 22 columns covering mortality rates, immunization coverage, HIV/AIDS, schooling, income, and GDP. Originally published on Kaggle by kumarajarshi, built from the WHO Global Health Observatory and UN data repositories: https://www.kaggle.com/datasets/kumarajarshi/life-expectancy-who
...
├── summative/
│   ├── linear_regression/
│   │   └── multivariate.ipynb   # EDA, feature engineering, model training and comparison
│   ├── API/
...
This runs on Render's free tier, so it spins down after a period of inactivity. The first request after a while can take up to 50 seconds to wake it back up. After that it responds normally.

- `POST /predict`: takes the 19 input indicators and returns a predicted life expectancy
- `POST /retrain`: upload a CSV in the same format as the original dataset and it retrains the model on the combined data
...
3. `flutter pub get`
4. Open `lib/main.dart` and check that `apiBaseUrl` points at the deployed API.
5. Run on a connected device or emulator: `flutter run`

It's one page: a field for each of the 19 model inputs, a Predict button, and a result area that shows the prediction or an error message if a value is missing or out of range.
